...
create unique index if not exists bookings_client_request_id_uniq
  on public.bookings (client_request_id)
  where client_request_id is not null;

create index if not exists bookings_active_guest_dates_idx
  on public.bookings (lower(guest_email), check_in_date, check_out_date, requested_unit_count)
...
begin
  ...
  where lower(guest_email) = v_guest_email
    ...
    and check_out_date = p_check_out_date
    and requested_unit_count = p_requested_unit_count
